...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		int syllCount = 0;
		boolean prevIsVowels = false;
		
		// each contiguous sequence of one or more vowels is a syllable 
		// loop last but one element
		for(int i=0; i<wordArr.length-1; i++) {
			if(isVowels(wordArr[i]) && !prevIsVowels) {
				syllCount++;
				prevIsVowels = true;
			}
			else if(!isVowels(wordArr[i])) {
				prevIsVowels = false;
			}
		}
		
		// but for the last letter in the world situation is different: "e" at the end of a word 
		// is not considered a syllable unless the word has no other syllables.
		char lastLetter = wordArr[wordArr.length-1];
		if((isVowels(lastLetter) && lastLetter!='e' && !prevIsVowels) || (lastLetter=='e' && syllCount==0))
			syllCount++;
	...
	/*
	 *  helper method check if char is vowels
	 *  @param char for checking
	 *  @return boolean true if is vowels, else false
	 */
	protected boolean isVowels(char ch) {
